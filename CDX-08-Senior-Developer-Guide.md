---
title: "08 Senior Developer Guide"
parent: "Codex CLI"
nav_order: 9
---
...
![Codex SDK als Puppenspieler-Metapher: Host-Prozess steuert die Codex-CLI als Marionette ueber JSONL-stdio-Events wie reasoning_delta, tool_call, file_change, message_delta und turn_completed](images/cdx-senior_01_codex_sdk.png)
...
![Subagents als Dirigent-mit-Orchester-Metapher: Haupt-Agent delegiert parallel an sec-reviewer, perf-reviewer und doc-writer und konsolidiert die Findings zu einer priorisierten Liste](images/cdx-senior_02_subagents.png)
...
![Verkehrsleitstelle mit Triage-Router-Agent, der Codex als function_tool oder Handoff-Target nutzt; umschlossen von Memory, Guardrails und Tracing sowie den 2026er Harness-Neuerungen](images/cdx-senior_03_agents_sdk_integration.png)
...
![Prompt-Architektur als 4-Bauteile-Blueprint (Rolle, Kontext, Goal/DoD, Output-Vertrag) plus Preambles-Format, Phase-Parameter-Fluss plan-act-summarize und Reasoning-Effort-Heuristik](images/cdx-senior_04_advanced_prompting.png)
...
3. **Goal / Constraints / DoD** (siehe [praktische_workflows](CDX-07-Praktische-Workflows) §1)
4. **Output-Vertrag** (Schema, Dateinamen, Commit-Konvention)

## 5. Observability, Tracing und Reproduzierbarkeit

![Observability-Radarschirm: Session-JSONL, Structured Logs, OTel-MCP und Reproduzierbarkeit speisen Loki, OpenSearch, Jaeger, Honeycomb und FinOps-Dashboards mit typischen Agent-Loop-Metriken](images/cdx-senior_05_observability.png)
...
![CI/CD als Fliessband mit Werkstaetten: Issue-Label-Trigger, Checkout, openai/codex-action, Cost-Gate und PR; parallel Matrix-Sub-Agenten pro Modul mit Aggregator-Job](images/cdx-senior_06_cicd_patterns.png)
...
![Enterprise-Control-Tower mit Shared-Cloud-Workspace in der Mitte und den Satelliten ZDR, SSO, SCIM, Audit-Logs, Model-Restriction und Custom-Responses-Proxy auf vier Integrations-Layern](images/cdx-senior_07_enterprise_betrieb.png)

- **Shared Codex Cloud Workspace**: zentrales Task-Monitoring, Quotas pro Team, Kostenabrechnung pro Workspace.
- **ZDR + SSO + SCIM + Audit-Logs** (siehe [sicherheit_und_sandboxing](CDX-04-Sicherheit-und-Sandboxing)).
...
![Dont-Board mit 8 roten Warnkarten (yolo-Laptop, AGENTS.md-Overflow, Trivial-Subagents, Prompt-lose Cloud-Tasks, never+danger-full-Kombi, Tests nach Implementation, keine Review-Pipeline, ueberladener Kontext) plus systemische Ursachen und Senior-Reflex](images/cdx-senior_08_anti_patterns.png)
...
![Cockpit-Bordkarte mit 11 goldenen Badges fuer config.toml-Profile, AGENTS.md, .codex/prompts, skills, MCP, GH-Action, notify, Session-ID, Cost-Gate, Trust-Levels und Observability-Stack - gegliedert nach Struktur, Integration, Kontrolle und Nachvollziehbarkeit](images/cdx-senior_09_senior_ready_checkliste.png)
...
- [vergleich_zu_alternativen](CDX-09-Vergleich-zu-Alternativen)
