...
Alle Beispiele gehen von `~/.codex/config.toml` mit Default-`gpt-5.3-codex` aus. Profile (`daily`, `planning`, `review`, `ci`) wie in [konfiguration_und_anpassung](CDX-03-Konfiguration-und-Anpassung) §3 empfohlen.

---

## 1. Ticket-Style-Prompt (das Grundprinzip)

![Jira-Ticket-Karte mit drei Pflichtblöcken Goal, Constraints, Definition of Done und konkretem OAuth-Beispiel](images/cdx-workflow_01_ticket_style_prompt.png)
...
![Schnellstart-Pipeline mit fünf Prompts (Scope, TDD, Implementation, CLI, Release) und Stoppuhr für 30-Minuten-Budget](images/cdx-workflow_02_greenfield_30min.png)
...
![Plan-Profil als Stadtführerin mit Lupe, strukturierter Summary-Output und Folge-Prompt für payments-Modul mit Mermaid-Sequenz](images/cdx-workflow_03_repo_verstehen.png)
...
![Verkehrsampel rot zu grün mit explizitem STOP-Checkpoint zwischen Failing-Test und Implementation, plus Regressions-Run](images/cdx-workflow_04_tdd_loop.png)
...
![Ermittler-Tafel mit Issue-Karte, failing Test, Red-Commit, Root-Cause-Patch und Coverage-Delta](images/cdx-workflow_05_bug_repro_fix.png)
...
Voraussetzung: GitHub-MCP-Server aktiv. Siehe [integrationen_ide_ci_cd](CDX-05-Integrationen-IDE-CI-CD) §5.

---

## 6. Security-Review eines Pull Requests

![Lupen-Patrouille mit Read-only-Sandbox, sieben OWASP-Kategorien und Severity-Skala info-low-med-high](images/cdx-workflow_06_security_review.png)
...
![Brücken-Sanierung React 18 zu 19 in zwei Phasen, Plan-Profil für Migrationsplan und Daily-Profil mit Stop-After-Each-Codemod](images/cdx-workflow_07_dependency_upgrade.png)
...
![Robot-Mechaniker im Pit-Stop, workflow_run-failure triggert codex-action mit workspace-write und drop-sudo, fixt und öffnet PR mit Backlink](images/cdx-workflow_08_autofix_bot.png)
...
![Vier-Augen-Prinzip mit Review-Stempel-Maschine: pull_request-Trigger, Codex-Action read-only mit prompt-file, review.md, Auto-Comment und versioniertes pr_review.md-Template](images/cdx-workflow_09_pr_review_action.png)
...
![Notify-Hook agent-turn-complete sendet JSON-Payload via curl an Slack-Channel — Webhook über Keychain/1Password injiziert](images/cdx-workflow_10_slack_notifier.png)
...
![Codex als MCP-Server (codex mcp serve --profile review) konsumiert von Claude via mcpServers-Konfiguration; bidirektionale Variante mit Claude als MCP-Server](images/cdx-workflow_11_multi_agent_codex_claude.png)
...
![codex cloud exec mit env, prompt-file und label; Cloud-VM läuft mehrere Stunden, danach cloud list/logs/pull; Plan-Markdown im Repo](images/cdx-workflow_12_cloud_multi_hour.png)
...
![Ticket-Prompt mit Goal/Constraints/DoD: Rename findById zu getUserById, Deprecated-Alias mit Warnung, src/ und docs/ aktualisieren, CHANGELOG-Eintrag](images/cdx-workflow_13_repo_weiter_refactor.png)
...
![Skill-Struktur skills/release/SKILL.md mit Front-Matter, Inputs $BUMP, 8 Schritte: Tree clean, Version bump, Update files, Tests, Changelog, Commit, Tag, Manual push](images/cdx-workflow_14_release_skill.png)
...
![main → hotfix-Branch mit on-request Approval und workspace-write Sandbox, Goal/Constraints (minimal change, Regression-Test, Sentry-ID), Definition of Done](images/cdx-workflow_15_hotfix_workflow.png)
...
![Sync von src/ Public API zu README und docs/, Constraints (kein neuer Abschnitt, Tonalität konsistent), DoD mkdocs build --strict](images/cdx-workflow_16_docs_sync.png)
...
![Werkzeuge: /status, codex usage, Modell-Wahl (5.3 vs. 5.4), Reasoning-Effort low/medium/high, /compact nach 20 Turns](images/cdx-workflow_17_cost_token_mgmt.png)
...
![~/.codex/config.toml-Profil local mit Ollama-Provider und llama-3.3-70b, ollama serve/pull, codex --profile local; Privacy-Use-Case und Limits](images/cdx-workflow_18_lokale_oss_modelle.png)
