---
title: "05 Integrationen IDE und CI/CD"
parent: "Codex CLI"
nav_order: 6
---
...
![IDE-Cockpit-Mockup mit Side-Panel-Chat, Diff-Viewer, Inline-Approval, Task-Tracker mit PR-Badges, Thinking-Panel und Cloud-Handoff sowie Hinweise auf VS Code, Cursor, Windsurf, JetBrains und geteilter ~/.codex/config.toml](images/cdx-integration_01_ide_extension.png)
...
![Terminal-Multiplexer als Schaltzentrale mit Codex im mittleren Pane, Shell-Completions für bash/zsh/fish/powershell und unterstützten Hosts tmux, Zellij, iTerm2, Ghostty, Warp Agent Mode inklusive Mouse-Support und Hover-Tooltips](images/cdx-integration_02_terminal_shell.png)
...
![Lokale Erde liefert Tasks an die Codex-Cloud-Raumstation mit isolierten Containern, Handoff-Befehlen cloud exec/list/logs/pull und Lizenzkarten für Plus, Pro, Business, Enterprise und Edu](images/cdx-integration_03_codex_cloud.png)
...
- **Plus, Pro, Business, Enterprise, Edu**: Cloud-Tasks inkludiert (plan-abhängige Limits; siehe [installation_und_setup](CDX-01-Installation-und-Setup)).
...
![GitHub-Octopus mit Codex-Armen: Codex Review App mit AGENTS.md/.codex/review.md, openai/codex-action@v1 mit allen Inputs und ein Autofix-Pattern via workflow_run plus failure-Bedingung](images/cdx-integration_04_github_integration.png)
...
**Minimaler Review-Job** siehe [sicherheit_und_sandboxing](CDX-04-Sicherheit-und-Sandboxing) §7.2.
...
![Codex als bidirektionaler USB-Hub: links MCP-Client-Ports zu filesystem, github, playwright, postgres, sentry, slack, linear und notion, rechts MCP-Server-Ports zu Claude Code, Cline, Zed und Windsurf, plus Approval-Modi und TOML-Snippets](images/cdx-integration_05_mcp.png)
...
![Signal-Kurier mit Funkverbindung in vier Stufen: agent-turn-complete-Event, JSON-Payload auf stdin, plattformspezifisches Custom-Skript (macOS afplay, Windows PowerShell, Linux Slack-Wrapper) und Targets Slack, Discord, Telegram, Desktop, CI-Ping, PagerDuty](images/cdx-integration_06_notify_hook.png)
...
![App-Store / Plug-in-Regal mit drei Reihen: wiederverwendbare Slash-Commands, Custom Tools und MCP-Server-Bundles, plus Distribution via codex marketplace add und Plugin-Skelett mit plugin.toml](images/cdx-integration_07_plugins.png)
...
![Zentrales Repository-Lager mit AGENTS.md, .codex/prompts/*.md und Config-Sharing als Regalböden, dazu Lieferdienste Codex, Cursor, Amp, Google Jules, Factory und Aider sowie eine Kompatibilitäts-Matrix](images/cdx-integration_08_editor_artefakte.png)
...
![Verkabeltes Architektur-Setup mit ~/.codex/config.toml (Notify, MCP github, MCP playwright mit approval never, profile ci) und Beispiel-Workflow Issue lesen, Bug reproduzieren, Fix, PR, Slack-Message](images/cdx-integration_09_beispiel_setup.png)
...
![Vier Pattern-Karten als Architektur-Galerie: Local-First Cloud-Burst, IDE-Zentral, Agent-gestütztes GitOps und MCP-Orchestrator (Variante Codex als Client und Codex als Server)](images/cdx-integration_10_architektur_muster.png)
